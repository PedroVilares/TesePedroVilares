...
def image_mover(df,folder_name,downsample):
    ...
    for path in image_paths:
        ...
        if 'R' in image_views[n]:
            ...
            flip = False
        if image_type == 'dcm':
            dicom_images(folder_name,path,n,flip,downsample)
        else:
            raw_images(folder_name,path,n,flip,downsample)
        new_image_paths.append(n)
        print('Image',n,'saved!')
        n +=1
    
    index_df = pd.DataFrame({'Image':new_image_paths,'Original':image_paths})
    csv_name = 'cross validation/image_data/'+folder_name.split('/')[2]+'/csv/'+ folder_name.split('/')[4]+'_'+folder_name.split('/')[5]+'.csv'
    index_df.to_csv(csv_name,index=False)

def raw_images(save_path,image_path,n,flip,downsample):
    try:
        ...
        if downsample:
            raw_mammogram_array_cropped = raw_array[:,:raw_array.shape[1]-600]
...
def dicom_images(save_path,image_path,n,flip,downsample):
    try:
        ...
        if downsample:
            raw_mammogram_array_cropped = cl_img[:,:cl_img.shape[1]-600]
            downsampled_image = resize(
                image= raw_mammogram_array_cropped,
                output_shape= (1500,1100),
                preserve_range= True,
...
def breast_area_factor(image_path):

    raw_mammogram_array = raw_mammogram(image_path)
    binarized_array = binarize_breast_region(raw_mammogram_array)
    patch_size = 300
    breast_number = np.sum(binarized_array)
    factor = format(breast_number/(patch_size*patch_size),'.1f')
...
def generator_transfer(size1,size2,aug,s):
    ...
    training_dir = 'image_data/downsampled/training_data/'
    validation_dir = 'image_data/downsampled/validation_data/'
...
def generator_fe1(size1,size2):

    data_generator = ImageDataGenerator(rescale= 1./255)

    data = data_generator.flow_from_directory(
        directory = 'D:/Full Mammographies/image_data/raw/validation',
        batch_size= 12,
        target_size= (size1,size2),
        class_mode= None,
        shuffle = False,
...
def create_trans_model(size1,size2):
    ...
    resnet_model = keras.applications.ResNet50V2(include_top=False,weights='imagenet',input_shape=(size1,size2,3))
...
def create_feature_extraction(size1,size2):

    feature_extractor = Sequential()

    #first block
    feature_extractor.add(Conv2D(32,kernel_size=(3, 3),strides=(2,2),input_shape=(size1,size2,1),activation='relu'))
    feature_extractor.add(MaxPooling2D(pool_size=(2,2),strides=(2,2)))

    #second block
    feature_extractor.add(Conv2D(32,kernel_size=(3, 3),strides=(2, 2),activation='relu'))
    feature_extractor.add(MaxPooling2D(pool_size=(2,2),strides=(2,2)))

    #third block
    feature_extractor.add(Conv2D(64,kernel_size=(3, 3),strides=(1, 1),activation='relu'))
    feature_extractor.add(MaxPooling2D(pool_size=(2,2),strides=(2, 2)))

    #Fully Connected Layer
    feature_extractor.add(Flatten())

    resnet = keras.applications.ResNet101V2(include_top=False,weights='imagenet',input_shape=(size1,size2,3))
    resnet.trainable = False
    feature_extractor1 = Sequential()
    feature_extractor1.add(resnet)
    feature_extractor1.add(Flatten())

    return feature_extractor,feature_extractor1
...
def copy_negatives(original_folder):
    ...
    for image_path in ps:
        ...
        #    preserve_range= True,
        #    anti_aliasing= True)
        save_path = original_folder + '/' + str(n) + 'n.bmp'
...
def generator_crossval(size1,size2,path,aug=False,s=False):
...
def classify_mammograms(model,dataframe,path,comp=True):

    split = path.split('/')[2]
    images_path = 'cross validation/image_data/'+split+'/downsampled'
    training_gen,validation_gen = generator_crossval(1500,1100,images_path,False,False)
    predictions1 = model.predict(training_gen,verbose=1)
    predictions2 = model.predict(validation_gen,verbose=1)
    predictions_image=[]
    for i in range(len(predictions1)):
        predictions_image.append(float("{:.3f}".format(predictions1[i][1])))
    for i in range(len(predictions2)):
        predictions_image.append(float("{:.3f}".format(predictions2[i][1])))
...
def patches_by_image(image_path,patch_size,overlap):
    ...
    n=0
    try:
        folder = image_path.split('/')[0] +'/'+ image_path.split('/')[1]+'/' + image_path.split('/')[2] + '/patches/' + image_path.split('/')[4] + '/' + image_path.split('/')[5]+ '/' + image_path.split('/')[6].split('.')[0]
        os.mkdir(folder)
...
def classify_patches(original_folder):
    ...
    for image_path in ps:
        print('Image ',image_path.split('/')[6].split('.')[0])
        folder = image_path.split('/')[0] +'/'+ image_path.split('/')[1]+'/' + image_path.split('/')[2] + '/patches/' + image_path.split('/')[4] + '/' + image_path.split('/')[5]+ '/' + image_path.split('/')[6].split('.')[0]
        l = os.listdir(folder)
...
def add_to_final_csv(split,csv_path):
    
    features_df1 = pd.read_csv('cross validation/numerical_data/split'+str(split)+'/features_training_normal.csv')
    features_df2 = pd.read_csv('cross validation/numerical_data/split'+str(split)+'/features_training_suspicious.csv')
    features_df3 = pd.read_csv('cross validation/numerical_data/split'+str(split)+'/features_validation_normal.csv')
    features_df4 = pd.read_csv('cross validation/numerical_data/split'+str(split)+'/features_validation_suspicious.csv')

    ratio1 = list(features_df1['Ratio'])
    ratio2 = list(features_df2['Ratio'])
