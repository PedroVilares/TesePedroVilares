...
def breast_area_factor(patient,image_view):

    image_path = "patients/"+patient+"/"+image_view+".bmp"
    raw_mammogram_array = raw_mammogram(image_path)
    binarized_array = binarize_breast_region(raw_mammogram_array)
    patch_size = 300
    ...
    for image in image_paths:
        ...
        if patient_label == 'Suspicious':
            ...
            for line in df_lines.index:
                ...
                line_side = 15
                half_side = int(patch_size/2)
                r = p.Rectangle((x_center-half_side,y_center-half_side),patch_size,patch_size,linewidth = 3,edgecolor = 'r',facecolor = 'None')
                
            if image_view in vertexes.keys():
                patches_vertexes = vertexes[image_view]
                overlays = overlay(x_center,y_center,patches_vertexes)
                line_side = 15
                for vertex in patches_vertexes:
                    mammogram[vertex[0]:vertex[1],vertex[3]-line_side:vertex[3]+line_side] = 0 
                    mammogram[vertex[0]-line_side:vertex[0]+line_side,vertex[2]:vertex[3]] = 0 
            ...
            if rows == 1:
                if cols == 1:
                    ...
                    s.add_patch(r)
                    s.set_title(overlays)
                else:    
                    s[i%cols].imshow(mammogram,cmap='gray')
                    s[i%cols].add_patch(r)
                    s[i%cols].set_title(overlays)
            else:
                s[i//cols,i%cols].imshow(mammogram,cmap='gray')
                s[i//cols,i%cols].add_patch(r)
                s[i//cols,i%cols].set_title(overlays)
...
def overlay_patient(patient,patch_size=300,overlap=0.2,threshold=0.15):
    
    patient_folder = 'patients/'+str(patient)+'/'
    image_paths = os.listdir(patient_folder)
    not_images = []
    for image in image_paths:
        if 't' in image:
    ...
    for image in image_paths:
        image_view = image.split('.')[0]
        if patient_label == 'Suspicious':
            path = 'D:/Architecture/patients/'+patient+'/'+image
            mammogram = raw_mammogram(path)
            df_lines = patient_df[patient_df['image view'] == image_view]
            for line in df_lines.index:
                x_center = int(round(mammogram.shape[1]*df_lines['x_center'][line]))
            ...
            if image_view in vertexes.keys():
                patches_vertexes = vertexes[image_view]
                overlays = overlay(x_center,y_center,patches_vertexes)
                patient_list.append(overlays)
                
    return patient_list

def overlay(x_center,y_center,sus_vertexes):
    
    xGT = x_center
    yGT = y_center
    side_patch = int(300/2)
    overlay_img = []
    ...
    overlays = float("{:.2f}".format(sum(overlay_img)/len(overlay_img)))
    return overlays
