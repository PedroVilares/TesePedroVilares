...
def evaluate_predictions_case(transfer):
    ...
    accuracy = accuracy_score(ground_truth,classifications)
    fpr,tpr,_= roc_curve(ground_truth,classifications)
    c_matrix = confusion_matrix(ground_truth,classifications,labels=[0,1])
    plt.plot(fpr,tpr)

    return auc, accuracy, df, c_matrix

def evaluate_predictions_image(transfer):
    ...
    auc = roc_auc_score(ground_truth,classifications)
    accuracy = accuracy_score(ground_truth,classifications)
    fpr,tpr,_= roc_curve(np.array(ground_truth),np.array(classifications),drop_intermediate=False)
    c_matrix = confusion_matrix(ground_truth,classifications)
    plt.plot(fpr,tpr)

    return auc, accuracy, fpr,tpr, df, c_matrix
...
def report_by_case(threshold,transfer):
    ...
    age = list(ground_truth_csv['age'])
    lesion = list(ground_truth_csv['lesion type'])
    ...
    for patient in patient_paths:
        ...
        lesion = patient_df['lesion type'][0]
        ...
        age_list.append(age)
        lesions_list.append(lesion)
        patches_list.append(num_patches)
        sus_patches_list.append(positive_patches)

        n += 1

    master_df = pd.DataFrame({'Patient': patient_list,'Age':age_list,'Ground Truth': gt_list,'Density': density_list,'Lesion':lesions_list,'Patches': patches_list,'Suspicious': sus_patches_list})    
    if transfer:
...
def report_by_image(threshold,transfer):
    ...
    for patient in patient_paths:
        ...
        for df_tuple in df_list:
            ...
            area_factor = patcher.breast_area_factor(patient,image_view)
            num_patches = len(predictions)
            positive_patches = 0
            for pred in predictions:
                if pred > threshold:
            ...
            area_factor_list.append(area_factor)
            lesions_list.append(lesion[i])
            age_list.append(age[i])
            i += 1
        n+=1

    master_df = pd.DataFrame({'Patient': patient_list,'Age':age_list,'Ground Truth': gt_list,'Density': density_list,'Lesion':lesions_list,'Image View':image_views_list,'Area Factor':area_factor_list,'Patches': patches_list,'Suspicious': sus_patches_list})    
    if transfer:
...
def make_hist(patient_number):
    ...
    plt.hist(x=predictions,bins=20,rwidth=0.9)
